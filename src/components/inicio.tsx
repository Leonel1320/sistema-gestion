...
export default function Inicio({ setVistaActual }: InicioProps) {
  // Estado local para forzar que el nombre sea reactivo si cambia en el almacenamiento
  const [nombreEmpresa, setNombreEmpresa] = useState(() => {
    const local = localStorage.getItem('empresa');
    if (local) {
      try { return JSON.parse(local).nombre || empresaDefault.nombre; } catch(e) {}
    }
    return empresaDefault.nombre;
  });
  ...
  // Cada vez que la pantalla de Inicio se muestra, re-comprueba si cambió el nombre en el modal
  useEffect(() => {
    cargarDatosDashboard();
    const local = localStorage.getItem('empresa');
    if (local) {
      try { setNombreEmpresa(JSON.parse(local).nombre || empresaDefault.nombre); } catch(e) {}
    }
  }, []);
    ...
    <div className="flex-1 p-8 bg-dark-bg overflow-y-auto select-none">
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        <div className="bg-dark-panel border border-dark-border p-6 rounded-2xl flex flex-col justify-between shadow-lg shadow-black/20 group hover:border-emerald-500/30 transition-colors">
          ...
          <div><h2 className="text-2xl font-black text-white leading-tight">${metricas.ventasMes.toLocaleString('es-AR')}</h2></div>
        </div>
        ...
        <div className="bg-dark-panel border border-dark-border p-6 rounded-2xl flex flex-col justify-between shadow-lg shadow-black/20 group hover:border-amber-500/30 transition-colors">
          ...
          <div><h2 className="text-3xl font-black text-white">{metricas.pendientes}</h2></div>
        </div>
        ...
        <div className="bg-dark-panel border border-dark-border p-6 rounded-2xl flex flex-col justify-between shadow-lg shadow-black/20 group hover:border-rose-500/30 transition-colors">
          ...
          <div><h2 className="text-3xl font-black text-white">{metricas.stockBajo} {metricas.stockBajo === 1 ? 'Material' : 'Materiales'}</h2></div>
        </div>
        ...
        <div className="bg-dark-panel border border-dark-border p-6 rounded-2xl flex flex-col justify-between shadow-lg shadow-black/20 group hover:border-blue-500/30 transition-colors">
          ...
          <div><h2 className="text-3xl font-black text-white">{metricas.totalProductos}</h2></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-8">
        <div className="col-span-2 bg-dark-panel border border-dark-border rounded-2xl p-6 shadow-xl">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => irAVista('ventas')} className="text-xs font-semibold text-emerald-400 hover:text-emerald-300 flex items-center gap-1 transition-colors">
                ...
                return (
                  <div key={idx} className="flex items-center justify-between p-4 bg-dark-bg border border-dark-border/40 rounded-xl text-sm hover:bg-dark-hover transition-all">
                    <div className="flex items-center gap-4">
                      <div className={`p-2 rounded-lg ${item.estado === 'CONFIRMADA' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'}`}><Clock size={16} /></div>
                      <div>
                        <p className="font-bold text-white">{item.estado === 'CONFIRMADA' ? 'Venta confirmada' : 'Nueva cotización creada'}</p>
        ...
        <div className="col-span-1 bg-dark-panel border border-dark-border rounded-2xl p-6 shadow-xl flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-3">
              <button onClick={() => irAVista('carrito')} className="w-full flex items-center justify-center gap-2 py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-sm font-bold transition-all shadow-md shadow-emerald-950/20 active:scale-[0.98]"><PlusCircle size={16} /> + Nueva Cotización</button>
              <button onClick={() => irAVista('productos')} className="w-full flex items-center justify-center gap-2 py-3 bg-dark-input hover:bg-dark-hover border border-dark-border text-gray-300 rounded-xl text-sm font-semibold transition-all active:scale-[0.98]"><PackagePlus size={16} /> + Cargar Producto</button>
              <button onClick={() => irAVista('clientes')} className="w-full flex items-center justify-center gap-2 py-3 bg-dark-input hover:bg-dark-hover border border-dark-border text-gray-300 rounded-xl text-sm font-semibold transition-all active:scale-[0.98]"><UserPlus size={16} /> + Registrar Cliente</button>
            </div>
          </div>
          <div className="text-[10px] text-gray-600 text-center mt-6 border-t border-dark-border/40 pt-4">
            Sistema {nombreEmpresa.split(' ')[0]} ERP v1.4 • Conectado a Postgres Local
          </div>
        </div>
      </div>
    </div>
