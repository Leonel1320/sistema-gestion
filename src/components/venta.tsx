...
import { 
  Plus, Search, SlidersHorizontal, Trash2, Edit2, Minus,
  FileText, Box, CheckCircle2, Percent, AlertTriangle, X, Save, Printer
} from 'lucide-react';
...
export default function Ventas({ setVistaActual }: { setVistaActual?: (vista: string) => void }) {
  ...
  // Formulario de edición simple controlado (Sólo estado)
  const [formEditar, setFormEditar] = useState<any>({ id: '', cliente: '', estado: '' });

  // 2. CARGA DE DATOS DESDE EL BACKEND (POSTGRESQL)
  ...
  // 3. ANULAR / ELIMINAR COTIZACIÓN (RESTABLECIDO)
  ...
  // 4. LÓGICA DE EDICIÓN SIMPLIFICADA (SÓLO ESTADO DE OPERACIÓN)
  ...
  // GUARDAR CAMBIOS (Apunta únicamente a modificar el estado en Postgres)
  const guardarEdicionVenta = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (respuesta.ok) {
        ...
        cargarDatosServidor(); // Recarga la grilla lateral reflejando el nuevo estado
      } else {
  ...
  return (
    <div className="flex flex-1 overflow-hidden bg-dark-bg relative">
      ...
      {/* INTERFAZ OSCURA DE PANTALLA (SE ESCONDE AL IMPRIMIR) */}
          ...
          <>
            <div className="p-6 border-b border-dark-border flex justify-between items-center bg-dark-panel">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => abrirModalEditar(ventaActiva)} className="p-2 bg-blue-600/10 text-blue-400 border border-blue-500/20 rounded-lg" title="Cambiar Estado"><Edit2 size={16} /></button>
                {/* BOTÓN ELIMINAR RESTABLECIDO CORRECTAMENTE AQUÍ */}
                <button onClick={() => setModalEliminarAbierto(ventaActiva.id)} className="p-2 bg-rose-600/10 border border-rose-500/20 text-rose-400 hover:bg-rose-600/20 rounded-lg transition-colors" title="Anular Cotización"><Trash2 size={16} /></button>
              </div>
            </div>
      ...
      {/* ZONA DE IMPRESIÓN COMPLETA (FIXED INDEPENDIENTE) */}
      {ventaActiva && (
        <div className="hidden print:block bg-white text-black p-12 font-sans fixed inset-0 z-[99999] h-screen w-screen bg-white">
          <div className="flex justify-between items-start border-b-2 border-gray-800 pb-6 mb-8">
            <div>
              ...
              <p className="text-[11px] text-gray-700 mt-2">Ruta 9, Acceso Sur — San Salvador de Jujuy</p>
            ...
            <div className="flex flex-col gap-4 items-end">
              ...
              <div className="border border-gray-400 rounded p-3 w-72 text-left">
                <h3 className="bg-gray-100 font-bold text-[10px] uppercase p-1 mb-2 text-center border-b border-gray-400">DATOS DEL CLIENTE</h3>
                <p className="text-xs font-bold">{`${ventaActiva.nombre || ''} ${ventaActiva.apellido || ''}`.trim() || ventaActiva.cliente || 'Consumidor Final'}</p>
                <p className="text-xs">{ventaActiva.documento ? `CUIT/DNI: ${ventaActiva.documento}` : 'Consumidor Final'}</p>
                <p className="text-xs">{ventaActiva.direccion || 'San Salvador de Jujuy'} {ventaActiva.barrio ? `- B° ${ventaActiva.barrio}` : ''}</p>
                {ventaActiva.telefono && <p className="text-xs">Tel: {ventaActiva.telefono}</p>}
              </div>
            </div>
          </div>
            ...
            <div className="w-72 border border-gray-400 rounded overflow-hidden">
              <table className="w-full text-xs">
                <tbody>
                  ...
                  <tr className="bg-gray-900 text-white"><td className="p-2 font-extrabold text-sm uppercase text-left">Total Final</td><td className="p-2 font-extrabold text-right text-sm text-emerald-400">${Number(ventaActiva.total).toLocaleString('es-AR')}</td></tr>
                </tbody>
              </table>
            </div>
          </div>
      ...
      {/* MODAL EDICIÓN EXCLUSIVA DE ESTADO (SÚPER ÁGIL Y SIN ERRORES) */}
      {modalEditarAbierto && (
